...
export const TYPE_DOCS = {
  reserved: [
    ...
    { name: '_err', default: '—', desc: '_err=1 이면 스키마 대신 상태코드에 맞는 에러 본문({code, message, …}). 4xx·5xx 에서만. 필드 없이 ?_status=422&_err=1 만으로도 동작' },
...
const EN_RESERVED: Record<string, { default?: string; desc: string }> = {
  ...
  _err: { desc: '_err=1 returns an error body matching the status code ({code, message, …}) instead of the schema. 4xx/5xx only. Works with no fields at all: ?_status=422&_err=1' },
