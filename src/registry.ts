...
export const TYPE_DOCS = {
  reserved: [
    ...
    { name: '_status', default: '200', desc: '강제 HTTP 상태코드. 4xx·5xx 인데 필드를 하나도 정의하지 않으면 상태코드에 맞는 기본 에러 본문({code, message, …})을 돌려준다 — 커스텀하려면 code·message 등을 필드로 정의하면 된다' },
...
const EN_RESERVED: Record<string, { default?: string; desc: string }> = {
  ...
  _status: { desc: 'Force the HTTP status code. On a 4xx/5xx with no fields defined you get a default error body matching the code ({code, message, …}) — define code/message as fields to customise it' },
