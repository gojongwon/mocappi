...
## 라우트 모드 — 베이스 URL 만 바꿔서 붙이기

쿼리스트링 없이 호출하는 방식. 워크스페이스에 프리셋을 저장하면 그 **경로가 라우트로 등록**된다.

```js
const API = 'https://mocappi.gojongwon.workers.dev/w/team123x';  // ← 이 줄만 바꾸면 끝

fetch(`${API}/users`)                            // GET  → 목록
fetch(`${API}/users`, { method: 'POST', body })   // POST → 생성된 아이템 (201 + id)
fetch(`${API}/users/42`)                         // GET  → 상세
```

- **와일드카드**: 경로를 `users/*` 로 저장하면 `/users/42`, `/users/9` 가 모두 매치된다.
  걸린 세그먼트는 `_seed` 로 넘어가므로 **id 별로 다른 데이터**가 나온다. 정확 일치가 항상 우선이고,
  그다음 `*` 가 적은 라우트가 이긴다
- **요청 쿼리가 프리셋보다 우선**이라 앱이 `?_page=2` 를 붙이는 건 그대로 동작한다
- **실패 모드**: GUI 워크스페이스 패널에서 `실패 모드`를 고르면 그 워크스페이스의 모든 라우트가
  해당 상태코드 + 에러 본문으로 응답한다. **앱 코드는 한 줄도 고치지 않는다** — 토스트·리트라이 UI 를
  실제 화면에서 테스트할 수 있다. API: `POST /schema/mode` `{ws, status}` (해제는 `null`)
- 라우트는 워크스페이스 기능이다 (공용 풀에는 없음). 저장소(D1/KV) 바인딩이 필요하고, 요청당 조회 1회가
  붙는다. `/api/*` 는 그대로 저장소 없이 순수 CPU 로 동작한다
