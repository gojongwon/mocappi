...
- 바디는 **스키마 타입으로 검증**된다: 스키마 필드와 JS 타입이 다르면 400 (값 범위·enum
  멤버는 검사하지 않는다 — 생성 규칙이지 검증 규칙이 아니다). 스키마에 없는 필드는
  entity 에서 조용히 제외된다 — `password` 같은 요청 전용 필드가 응답에 새지 않는다.
- `PATCH` 는 보낸 필드만 병합하고, `PUT` 은 **완전한 표현**을 요구한다 — `id` 를 뺀 전 필드가
  없으면 400 (빠진 필드 목록 + "부분 수정은 PATCH" 힌트). nullable(`?`) 필드만 생략 가능하고
  생략하면 `null` 로 저장돼, PUT 뒤에도 아이템 모양이 항상 스키마와 일치한다.
...
- GUI: 프리셋을 불러오고 쓰기 메서드를 고르면 URL 아래 **상태 패널**이 나타난다 —
  바디를 적어 보내면 성공 시 GET 목록으로 전환돼 변화가 바로 보인다.
